...
/**
 * Use this class's 'getItem()' method to retrieve ItemStacks from the 'Mekanism' class.
 * 
 * @author AidanBrady
 *
 */
public final class ItemRetriever {
     ...
     *
     * Mekanism identifiers follow an easy-to-remember pattern. All identifiers are identical to the String returned by
     * 'getItemName().' None include spaces, and all start with a capital letter. The name that shows up in-game can be
     * stripped down to identifier form by removing spaces and all non-alphabetic characters (,./'=-_). Below is an
     * example:
     *
     * ItemStack enrichedAlloy = ItemRetriever.getItem("EnrichedAlloy");
     *
     * Note that for items or blocks that have specific metadata you will need to create a new ItemStack with that
     * specified value, as this will only return an ItemStack with the meta value '0.'
     *
     * Make sure you run this in or after FMLPostInitializationEvent runs, because most items are registered when
     * FMLInitializationEvent runs. However, some items ARE registered later in order to hook into other mods. In a rare
     * circumstance you may have to add "after:Mekanism" in the @Mod 'dependencies' annotation.
     *
