...
public abstract class TileMachineFluidIn extends TileMachineBase implements IFluidHandler {
    ...
    public TileMachineFluidIn(String localName, int energyMax, int fluidMax, int inSize, int outSize,
            boolean usesUpgrades, boolean usesEnergy, double pt, double er, Fluid flType, int flAmount, int smod,
            int tmod, NCRecipeHelper ncrecipes) {
    ...
    public boolean canCook() {
        ...
        if (getEnergyRequired > this.energyStorage.getMaxEnergyStored() && cookTime <= 0
                && this.energyStorage.getEnergyStored() < this.energyStorage.getMaxEnergyStored()
                        - (int) Math.ceil(getEnergyRequired / getProcessTime)) {
            flag = false;
            return false;
        }
        if (getEnergyRequired < this.energyStorage.getMaxEnergyStored() && cookTime <= 0
