...
public class TileSynchrotron extends TileInventory implements IEnergyHandler, IEnergyReceiver, ISidedInventory {
    ...
    /*
     * private void place(int x, int y, int z) { ForgeDirection forward =
     * ForgeDirection.getOrientation(this.getBlockMetadata()).getOpposite(); int xc = xCoord + 2*forward.offsetX; int yc
     * = yCoord + y + 3; int zc = zCoord + 2*forward.offsetZ; if (this.getBlockMetadata() == 4) worldObj.setBlock(xc+x,
     * yc, zc+z, Blocks.coal_block); else if (this.getBlockMetadata() == 2) worldObj.setBlock(xc-z, yc, zc+x,
     * Blocks.iron_block); else if (this.getBlockMetadata() == 5) worldObj.setBlock(xc-x, yc, zc-z, Blocks.gold_block);
    ...
    private void power() {
        if (storage.getEnergyStored() >= (int) (1000 * ((200 - (efficiency / 10000)) / 100))
                ...
                && percentageOn > 0
                && fuel - length >= 0) {
            fuel = fuel - (fuel < fuelMax / 2 ? length * ((fuel * 2) + 7500) / fuelMax
                    : length * (fuelMax + 7500) / fuelMax) / 4;
            ...
            if (length > 0) radiationPower = (fuel < fuelMax / 2 ? (fuel * 2) / fuelMax : 1)
                    * (2 * 16
                            * (((length * 100) + 1) / 100)
                            * (((length * 100) + 1) / 100)
                            * percentageOn
                                                                    ...
                                                                    * Math.pow(10, 5.5)
                                                                    - 3 * c
                                                                            ...
                                                                            * (2 * 16
                                                                                    ...
                                                                                    * (((length * 100) + 1) / 100)
                                                                                    * percentageOn
                                                                                    * (NuclearCraft.superElectromagnetRF
                                                                                            / 100)
                                                                                    * (efficiency / 10000))
                                                                            * Math.pow(10, 10))
                                                                    / (8 * e * e * c * k * Math.pow(10, -15) - 3
                                                                            * (length * 100)
                                                                            * (length * 100)
                                                                            * (2 * 16
                                                                                    * (((length * 100) + 1) / 100)
    ...
    /*
     * private void power() { double efficiency1 = efficiency/10000; if (storage.getEnergyStored() >= (int)
     * (1000*((200-efficiency1)/100)) && worldObj.isBlockIndirectlyGettingPowered(this.xCoord, this.yCoord, this.zCoord)
     * && flag && percentageOn > 0 && fuel - length >= 0) { fuel = fuel - (fuel < fuelMax/2 ?
     * length*((fuel*2)+7500)/fuelMax : length*(fuelMax+7500)/fuelMax)/4; this.storage.receiveEnergy(- (int)
     * (1000*((200-efficiency1)/100)), false); if (efficiency >= 1000000) efficiency = 1000000; else efficiency =
     * efficiency + 20; double radPow =
     * (20*16*(length+1)*(length+1)*percentageOn*NuclearCraft.superElectromagnetRF*efficiency1)/10000; if (length > 0)
     * radiationPower = (fuel < fuelMax/2 ? (fuel*2)/fuelMax : 1)*radPow/1000; // 20 Watts per RF/t int modifiedLength =
     * length*100; double a1 = modifiedLength*radPow/1000; double a2 = modifiedLength*modifiedLength*radPow/1000; double
     * a3 = 0.5; double a4 = length*length*radPow/1000; double velocitysq = (a2-a1)/(a4-a3); double gamma = 1 +
     * velocitysq; particleEnergy = (m*c*c*(gamma-1)*Math.pow(10, 4))/(1000*e); } else { if (efficiency > 2500)
     * efficiency = efficiency - 2500; else efficiency = 0; radiationPower = 0; particleEnergy = 0; } }
     */
