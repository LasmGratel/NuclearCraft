...
public class TileSteamDecompressor extends TileEntity implements IFluidHandler {
    ...
    public void steam() {
        if (tank.getFluidAmount() != 0) {
            if (tank.getFluid().getFluid() != NuclearCraft.denseSteam
                    && tank.getFluid().getFluid() != FluidRegistry.getFluid("denseSteam")) {
                tank.drain(NuclearCraft.steamDecompressRate, true);
            } else if (tank.getFluid().getFluid() == NuclearCraft.denseSteam
                    || tank.getFluid().getFluid() == FluidRegistry.getFluid("denseSteam")) {
