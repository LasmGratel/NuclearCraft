...
public class TileDenseSteamDecompressor extends TileEntity implements IFluidHandler {
    ...
    public void steam() {
        if (tank.getFluidAmount() != 0) {
            if (tank.getFluid().getFluid() != NuclearCraft.superdenseSteam
                    && tank.getFluid().getFluid() != FluidRegistry.getFluid("superdenseSteam")) {
                tank.drain(NuclearCraft.steamDecompressRate, true);
            } else if (tank.getFluid().getFluid() == NuclearCraft.superdenseSteam
                    || tank.getFluid().getFluid() == FluidRegistry.getFluid("superdenseSteam")) {
